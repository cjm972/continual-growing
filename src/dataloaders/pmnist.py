...
def get(data_path, seed, fixed_order=False, pc_valid=0, valid_split=0.0):
    ...
    binary_pmnist_path = os.path.join(data_path, 'binary_pmnist')
    if not os.path.isdir(binary_pmnist_path):
        os.makedirs(binary_pmnist_path)
        # Pre-load
        # MNIST
        mean = (0.1307,)
        std = (0.3081,)
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                ...
                torch.save(data[i][s]['x'], os.path.join(binary_pmnist_path, 'data' + str(r) + s + 'x.bin'))
                torch.save(data[i][s]['y'], os.path.join(binary_pmnist_path, 'data' + str(r) + s + 'y.bin'))
        print()
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                data[i][s] = {'x': [], 'y': []}
                data[i][s]['x'] = torch.load(os.path.join(binary_pmnist_path, 'data' + str(r) + s + 'x.bin'))
